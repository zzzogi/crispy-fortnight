...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create a unique folder name using gift ID
    const folderName = `gifts/${slugify(giftName)}/`;
    ...
    // Return the first image as the main gift image URL
    return NextResponse.json({
      success: true,
...
export async function DELETE(request: NextRequest) {
  try {
    // For DELETE requests, get the giftId from the URL params
    ...
    // List all objects with the gift's folder prefix
    const folderPrefix = `gifts/${slugify(giftName)}/`;
    ...
    if (!listedObjects.Contents || listedObjects.Contents.length === 0) {
      ...
      return NextResponse.json({
        success: true,
        message: "No images found for this gift",
      });
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully deleted ${listedObjects.Contents.length} images for gift ${giftName}`,
    });
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    // For GET requests, get the giftId from the URL params
    const { searchParams } = new URL(request.url);
    ...
    // List all objects with the gift's folder prefix
    const folderPrefix = `gifts/${giftId}/`;
